    ...
    try {
        ...
        while (retries < maxRetries) {
            try {
                ...
                const from_data = await axios.get(`https://api.coingecko.com/api/v3/coins/${fromCurrency}/history?date=${date}`);
                const from_data_main = from_data.data;
                let price_1 = from_data_main?.market_data?.current_price?.usd ;

                const to_data = await axios.get(`https://api.coingecko.com/api/v3/coins/${toCurrency}/history?date=${date}`);
                const to_data_main = to_data.data;
                let price_2 = to_data_main?.market_data?.current_price?.usd ;
                let price_day = price_1/price_2;


                res.json({
                    success: true,
                    message: `On this ${date} the price of one ${fromCurrency} is equals to ${price_day}  ${toCurrency}`,
                    data: {
                        fromCurrency,
                        toCurrency,
                        price_day,
